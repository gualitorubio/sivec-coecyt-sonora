...
# Usamos un bloque try/except para capturar errores de conexión al inicio
try:
    client = genai.Client(api_key=st.secrets["GEMINI_API_KEY"])
    supabase = create_client(st.secrets["SUPABASE_URL"], st.secrets["SUPABASE_KEY"])
except Exception as e:
    st.error(f"Error de inicialización de clientes: {e}")
            ...
            with st.status(" 🛸 Procesando en infraestructura de Rubio Intelligence Systems...", expanded=True) as status:
                try:
                    ...
                    response = client.models.generate_content(
                        ...
                        contents=f"Contexto: {rama_cientifica}. Palabras clave: {termino_busqueda}. Pregunta: {pregunta_usuario}"
                    )
